...
math_prompt = """
This is a math question. The focus is on **complex calculations** that require **multiple steps** and **critical reasoning**. Do not ask for methods, names of formulas, or explanations. Avoid decimal numbers that require a calculator.
"""
...
# Energy prompt focuses on cutting-edge research at national labs
energy_prompt = f"""This is an energy question. Mention research at a national lab (e.g., Fermilab, Sandia, Argonne) 
then ask a **challenging** {energy_topics[x]} question related to that research. Follow Science Bowl rules strictly."""

def generate_science_bowl_question(retrieved_text, difficulty, category, topic):
    """ Generates a high-quality, strictly relevant Science Bowl question. """
    if not retrieved_text:
        return "Error: No valid context retrieved for question generation."

    prompt = energy_prompt if category == "Energy" else math_prompt if category == "Math" else ""

    prompt += f"""
    Generate a Science Bowl question in the category: {category}, subtopic: {topic}.
    
    🔹 **STRICT RULES:**
    - **DO NOT** use conversational phrases like "Certainly" or "According to..."
    - **DO NOT** create True/False questions.
    - **DO NOT** use information outside the provided context.
    - **DO NOT** include decimal numbers that require a calculator.

    🔹 **Question Structure:**
    - **Toss-up:** Must require **deep thinking** or **multiple steps**. Answer in ≤5 seconds.
    - **Bonus:** Should be **significantly harder**. Answer in ≤20 seconds.
    - **50% short answer, 50% multiple choice.** Ensure multiple-choice questions use **W, X, Y, Z**.
    - **Short answers must be a single word, a number, or a term** (NO explanations!).
    
    🔹 **Make It Harder:**
    - Questions should **force competitors to combine concepts** instead of just recalling facts.
    - Avoid simple definition-based questions (e.g., "What is the symbol for moment of inertia?").
    - If asking for a calculation, **choose whole numbers or simple fractions**.

    🔹 **USE ONLY THIS CONTEXT (STRICTLY REQUIRED!):**
    {retrieved_text}

    🔹 **Format:**
    TOSS-UP: [Question here]
    ANSWER: [Single-word answer or multiple-choice]
...
def verify_science_bowl_question(question):
    """ Ensures question follows competition rules and makes it harder if needed. """
    verification_prompt = f"""
    Validate and enhance this Science Bowl question:
    {question}

    🔹 **STRICT RULES:**
    - **Short answer questions**: Answer must be **a single word, number, or term**.
    - **Multiple-choice questions**: Must have **4 balanced choices labeled W, X, Y, Z**.
    - **Toss-up must be solvable in ≤5s, bonus in ≤20s**.
    - **No True/False questions.**
    - **Ensure difficulty is at an advanced high-school or early college level.**
    - **Remove decimal numbers if present.**

    Return the **corrected** and **more challenging** version of the question.
    """
...
def similarity(question, user_answer):
    ...
    - Return whether it is correct and an explanation of how to solve the question.
    """
...
if st.button("Generate Question"):
    ...
    index = y if category == "Energy" else next((i for i, t in enumerate(subtopics[category]) if t == topic), 0)

    ans = {"Chemistry": chem_text, "Physics": phys_text, "Biology": bio_text, "Math": [subtopics[category][index]], "Energy": [phys_text, chem_text, bio_text][x]}[category]

    retrieved_text = ans[index]
    raw_question = generate_science_bowl_question(retrieved_text, 3, category, topic)
    print(raw_question)
    st.session_state.question_final = verify_science_bowl_question(raw_question)
    print(st.session_state.question_final)

# Display Question
if st.session_state.question_final:
    tossup_index = st.session_state.question_final.find("ANSWER")
    bonus_index = st.session_state.question_final.find("BONUS")

    toss_up = st.session_state.question_final[:tossup_index].strip()
    bonus = st.session_state.question_final[bonus_index:].strip()
